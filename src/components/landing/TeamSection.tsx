      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {TEAM_MEMBERS.map((member) => (
            <div
              key={member.name}
              className="flex-shrink-0 w-[310px] snap-start group"
            >
              <div className=" bg-wenav-gray rounded-wenav overflow-hidden transition-all group-hover:ring-2 group-hover:ring-wenav-purple/30">
                ...
                <h3 className="font-semibold text-wenav-dark text-[13px] pl-1 mx-4 mb-1">
                  {member.name}
                </h3>
                <p className="text-[11px] text-gray-500 pl-1 mx-4 mb-5">{member.role}</p>
              </div>
            </div>
          ))}
        </div>
